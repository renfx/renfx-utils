...
public class AESUtil {
	...
	public static byte[] encrypt(String data, String key) {
		if (data == null) {
			throw new SecretException("Data is null");
		}

		if (StringUtils.isBlank(key) || key.length() != 16) {
			throw new SecretException("Key is null or key's length is not 16");
		}
		...
		try {
			...
			throw new SecretException("encrypt error", e);
		}
	}
	...
	public static byte[] decrypt(byte[] dataBytes, String key) {
		...
		try {
			...
			throw new SecretException("decrypt error", e);
		}
	}
